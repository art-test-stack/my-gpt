...
def forward_model(model, input_ids):
    ...
    # outputs = model(input_ids, target_ids, reduction='none').logits # (B, Seq, V)
    # 
    ...
    outputs = model(input_ids, target_ids, reduction='none') # (B, Seq) NATs
    losses = outputs.loss.view(batch_size, seq_len) # (B, Seq, V)
...
class ModelWrapper:
    """Lightweight wrapper to give HuggingFace models a gpt-lab-compatible interface."""
...
def load_hf_model(hf_path: str, device):
    ...
    tokenizer = Tokenizer.from_config(config=tokconfig)
...
def main():
    ...
    parser.add_argument('--run-name', type=str, default=None, help='gpt-lab model tag to identify the checkpoint directory')
    ...
    if is_hf_model:
        ...
        model, tokenizer, meta = load_model("base", device, phase="eval", run_name=args.run_name, step=args.step)
